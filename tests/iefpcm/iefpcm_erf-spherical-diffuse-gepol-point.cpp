...
SCENARIO("Test solver for the IEFPCM for a point charge in a spherical diffuse "
         "environment modelled with an error function profile and a GePol cavity",
         "[solver][iefpcm][iefpcm_erf-spherical-diffuse-gepol-point][anisotropic]") {
  GIVEN("An anisotropic environment modelled as a spherical diffuse permittivity") {
    ...
    WHEN("the spherical diffuse layer is centered at the origin and the charge is "
      ...
      int maxL = 5;
      SphericalDiffuse<OneLayerErf> gf_o(
    ...
    AND_WHEN("the spherical diffuse layers is centered away from the origin and the "
      ...
      int maxL = 5;
      SphericalDiffuse<OneLayerErf> gf_o(eps1, eps2, width, center, offset, maxL);
